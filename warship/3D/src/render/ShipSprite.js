...
const ShipSprite = {
  ...
  build(scene, ship) {
    ...
    // 左右舷装填状态灯（绿=就绪，灰=装填中）；作为船体层子节点随船旋转 → 永远贴在正确一侧
    ship.portInd = scene.add.circle(0, -B * 0.5, 4, 0x2ecc71).setStrokeStyle(1, 0x14401f);
    ship.starInd = scene.add.circle(0, B * 0.5, 4, 0x2ecc71).setStrokeStyle(1, 0x14401f);
    ship.hullLayer.add(ship.portInd);
    ship.hullLayer.add(ship.starInd);
  ...
  updateVisual(ship, tMs) {
    ...
    ship.shadow.setPosition(ship.x, ship.y + 6);
    ship.shadow.setDepth(ship.y - 1);
  },
};
