...
class Ship {
  constructor(scene, x, y, opts = {}) {
    ...
    // 船型大小缩放（供 3D 渲染按 maxHp 区分大船/小船）；2D 时代此值由 ShipSprite 用
    this.visScale = Util.clamp(Math.sqrt(this.maxHp / 100), 0.85, 1.5);
    this.mesh3d = null;   // 3D 网格由 GlobeRenderer 注入（BattleScene 内）
  }
  ...
  update(dt, wind) {
    ...
    // 3D 渲染（贴球面 + 切向朝向 + 舷侧装填指示灯）由 GlobeRenderer/ShipMesh 每帧 sync 完成，
    // 不在此处操作 2D 图形。尾迹由 BattleScene 按航速经 FX.spawnTrail 触发。

    // 尾迹节流计时（供 BattleScene 读取决定是否生成 3D 尾迹）
  ...
  // 开火：成功返回 {hit,quality,travelTime,side}，失败返回 null
  fire(side, target) {
    if (!Combat.canFire(this, target, side)) return null;
    this[side + 'Reload'] = C.RELOAD_TIME * this.reloadMul;
    const res = Combat.resolveVolley(this, target, side);
    res.travelTime = Combat.distance(this, target) / C.PROJECTILE_SPEED;
    res.side = side;
    return res;
  }
  ...
  takeDamage(d) {
    ...
    this._hitFlash = 0.18; // 3D 下由 ShipMesh 读取做受击染红
  }
}
